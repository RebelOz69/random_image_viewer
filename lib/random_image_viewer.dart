...
class RandomImageViewer extends StatefulWidget {
  ...
  final Function(double)? onRotationChanged;  // New callback for rotation changes
  ...
  const RandomImageViewer({
    ...
    this.onRotationChanged, this.progressIndicatorColor,
  });
...
class _RandomImageViewerState extends State<RandomImageViewer> {
  ...
  void initState() {
    super.initState();
    _rotation = widget.initialRotation * (math.pi / 180);  // Convert degrees to radians
  }
  ...
  Widget build(BuildContext context) {
      ...
      imageWidget = GestureDetector(
        onPanUpdate: (details) {
          if (details.delta.dx != 0 || details.delta.dy != 0) {
            ...
            final double angle = _calculateRotationAngle(details.localPosition, centerOffset, details.delta);

            setState(() {
              _rotation += angle;
              // Notify callback if provided
              if (widget.onRotationChanged != null) {
                widget.onRotationChanged!(_rotation * (180 / math.pi)); // Convert to degrees for callback
              }
            });
          }
        },
    ...
    Widget tappableImage = GestureDetector(
      onTap: widget.onTap,
      onDoubleTapDown: widget.doubleTapZoom ? (details) => _doubleTapDetails = details : null,
  ...
  double _calculateRotationAngle(Offset position, Offset center, Offset delta) {
    ...
    if (vector.distance > 0) {
      ...
      angle = (delta.dx / 150) * angleSign * math.pi / 32; // Adjust sensitivity here
    }
  ...
  Widget _buildPlaceholderWidget() {
    return widget.placeholderWidget ?? SizedBox(
      height: widget.loaderHeight,
      width: widget.loaderWidth,
      child: Center(
        child: CircularProgressIndicator(
  ...
  Widget _buildErrorWidget(BuildContext context) {
    return widget.errorWidget ?? Container(
      ...
      color: widget.backgroundColor,
      child: Center(
        child: Icon(widget.errorIcon ?? Icons.error, color: widget.errorColor ?? Colors.red, size: 48),
      ),
    );
  }
}
...
class RotationControls extends StatelessWidget {
  ...
  const RotationControls({Key? key, required this.viewerState}) : super(key: key);
